   ...
   lengthtime=length(time)
	for (i in 1:lengthtime){    #figure out how to make the rectangles
		...
		rectplot(-ser,bins,xmin,xmax,ylim,barcol1,barcol2)#make bar plot
                text(cbind(time[i]+3,max(bins)+(count%%2)*1.2),label=toString(dates$Date[count+1]),cex=.75,col="black")#add dates to things
              }
	}
   points(sdate,sML,col="black",pch=19) #These plots may need to be revisited.. however for the moment they are  good enough.
   points(curves$c[,2],curves$c[,3],pch=1 ,cex=.2,col="black")# make real growth curve!
   points(curves$c[,2],curves$c[,4],cex=.01,col="pink")#show pink bined growth curve
