...
kscanplot <- function(window=window,z=zkscan){
    ...
    plot(z[nzero,2],z[nzero,1],type="l",xlab="K-parameter (year-1)",ylab="Goodness of fit (Rn)",xaxt="n",yaxt="n",log="x",ylim=c(0,max(z[,1])*1.1),col="grey")
...
fixedkscanplot <- function(window=window,z=fixzkscan){
    ...
    par(las=1,bty="n",xaxs="i",yaxs="i")
    ma <- movingAverage(z[nzero,1],window)
    plot(z[nzero,2],z[nzero,1],type="l",xlab="K-parameter (year-1)",ylab="Goodness of fit (Rn)",xaxt="n",yaxt="n",log="x",ylim=c(0,max(z[,1])*1.1),col="grey",pch=.8)
    axis(1,pos=0,tck=0.02)
    axis(2,tck=0.02,las=2)
    points(z[nzero,2],z[nzero,1],col="grey",cex=.9,pch=19)
